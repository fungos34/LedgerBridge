# Generated by Django 5.2.9 on 2026-01-12 01:14

import django.db.models.deletion
from django.conf import settings
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = [
        migrations.swappable_dependency(settings.AUTH_USER_MODEL),
    ]

    operations = [
        migrations.CreateModel(
            name='BankMatch',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('document_id', models.IntegerField(blank=True, null=True)),
                ('bank_reference', models.TextField()),
                ('bank_date', models.TextField()),
                ('bank_amount', models.TextField()),
                ('matched_at', models.TextField()),
            ],
            options={
                'verbose_name': 'Bank Match',
                'verbose_name_plural': 'Bank Matches',
                'db_table': 'bank_matches',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Extraction',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('document_id', models.IntegerField()),
                ('external_id', models.TextField(unique=True)),
                ('extraction_json', models.TextField(help_text='JSON extraction data')),
                ('overall_confidence', models.FloatField()),
                ('review_state', models.TextField()),
                ('created_at', models.TextField()),
                ('reviewed_at', models.TextField(blank=True, null=True)),
                ('review_decision', models.TextField(blank=True, null=True)),
                ('llm_opt_out', models.BooleanField(default=False)),
            ],
            options={
                'verbose_name': 'Extraction',
                'verbose_name_plural': 'Extractions',
                'db_table': 'extractions',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='FireflyCache',
            fields=[
                ('firefly_id', models.IntegerField(primary_key=True, serialize=False)),
                ('external_id', models.TextField(blank=True, null=True)),
                ('internal_reference', models.TextField(blank=True, null=True)),
                ('type', models.TextField()),
                ('date', models.TextField()),
                ('amount', models.TextField()),
                ('description', models.TextField(blank=True, null=True)),
                ('source_account', models.TextField(blank=True, null=True)),
                ('destination_account', models.TextField(blank=True, null=True)),
                ('notes', models.TextField(blank=True, null=True)),
                ('category_name', models.TextField(blank=True, null=True)),
                ('tags', models.TextField(blank=True, help_text='JSON array of tags', null=True)),
                ('synced_at', models.TextField()),
                ('match_status', models.TextField(default='UNMATCHED')),
                ('matched_document_id', models.IntegerField(blank=True, null=True)),
                ('match_confidence', models.FloatField(blank=True, null=True)),
                ('deleted_at', models.TextField(blank=True, null=True)),
            ],
            options={
                'verbose_name': 'Firefly Transaction Cache',
                'verbose_name_plural': 'Firefly Transaction Cache',
                'db_table': 'firefly_cache',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Import',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('external_id', models.TextField(unique=True)),
                ('document_id', models.IntegerField()),
                ('firefly_id', models.IntegerField(blank=True, null=True)),
                ('status', models.TextField()),
                ('error_message', models.TextField(blank=True, null=True)),
                ('payload_json', models.TextField()),
                ('created_at', models.TextField()),
                ('imported_at', models.TextField(blank=True, null=True)),
            ],
            options={
                'verbose_name': 'Import',
                'verbose_name_plural': 'Imports',
                'db_table': 'imports',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='InterpretationRun',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('document_id', models.IntegerField(blank=True, null=True)),
                ('firefly_id', models.IntegerField(blank=True, null=True)),
                ('external_id', models.TextField(blank=True, null=True)),
                ('run_timestamp', models.TextField()),
                ('duration_ms', models.IntegerField(blank=True, null=True)),
                ('pipeline_version', models.TextField()),
                ('algorithm_version', models.TextField(blank=True, null=True)),
                ('inputs_summary', models.TextField(help_text='JSON summary of inputs')),
                ('rules_applied', models.TextField(blank=True, help_text='JSON rules applied', null=True)),
                ('llm_result', models.TextField(blank=True, help_text='JSON LLM result if used', null=True)),
                ('final_state', models.TextField()),
                ('suggested_category', models.TextField(blank=True, null=True)),
                ('suggested_splits', models.TextField(blank=True, help_text='JSON splits if applicable', null=True)),
                ('auto_applied', models.BooleanField(default=False)),
                ('decision_source', models.TextField(blank=True, null=True)),
                ('firefly_write_action', models.TextField(blank=True, null=True)),
                ('firefly_target_id', models.IntegerField(blank=True, null=True)),
                ('linkage_marker_written', models.TextField(blank=True, null=True)),
                ('taxonomy_version', models.TextField(blank=True, null=True)),
            ],
            options={
                'verbose_name': 'Interpretation Run',
                'verbose_name_plural': 'Interpretation Runs',
                'db_table': 'interpretation_runs',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='Linkage',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('extraction_id', models.IntegerField(unique=True)),
                ('document_id', models.IntegerField()),
                ('firefly_id', models.IntegerField(blank=True, help_text='NULL for orphans', null=True)),
                ('link_type', models.TextField(default='PENDING', help_text='PENDING, LINKED, ORPHAN, or AUTO_LINKED')),
                ('confidence', models.FloatField(blank=True, help_text='Match confidence 0.0-1.0', null=True)),
                ('match_reasons', models.TextField(blank=True, help_text='JSON array of match reasons', null=True)),
                ('linked_at', models.TextField()),
                ('linked_by', models.TextField(blank=True, help_text='AUTO, USER, etc.', null=True)),
                ('notes', models.TextField(blank=True, null=True)),
            ],
            options={
                'verbose_name': 'Linkage',
                'verbose_name_plural': 'Linkages',
                'db_table': 'linkage',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='MatchProposal',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('firefly_id', models.IntegerField()),
                ('document_id', models.IntegerField()),
                ('match_score', models.FloatField()),
                ('match_reasons', models.TextField(blank=True, help_text='JSON array of reasons', null=True)),
                ('status', models.TextField(default='PENDING')),
                ('created_at', models.TextField()),
                ('reviewed_at', models.TextField(blank=True, null=True)),
            ],
            options={
                'verbose_name': 'Match Proposal',
                'verbose_name_plural': 'Match Proposals',
                'db_table': 'match_proposals',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='PaperlessDocument',
            fields=[
                ('document_id', models.IntegerField(primary_key=True, serialize=False)),
                ('source_hash', models.TextField()),
                ('title', models.TextField(blank=True, null=True)),
                ('document_type', models.TextField(blank=True, null=True)),
                ('correspondent', models.TextField(blank=True, null=True)),
                ('tags', models.TextField(blank=True, help_text='JSON array of tags', null=True)),
                ('first_seen', models.TextField()),
                ('last_seen', models.TextField()),
            ],
            options={
                'verbose_name': 'Paperless Document',
                'verbose_name_plural': 'Paperless Documents',
                'db_table': 'paperless_documents',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='VendorMapping',
            fields=[
                ('id', models.AutoField(primary_key=True, serialize=False)),
                ('vendor_pattern', models.TextField(unique=True)),
                ('destination_account', models.TextField(blank=True, null=True)),
                ('category', models.TextField(blank=True, null=True)),
                ('tags', models.TextField(blank=True, help_text='JSON array of tags', null=True)),
                ('created_at', models.TextField()),
                ('updated_at', models.TextField()),
                ('use_count', models.IntegerField(default=1)),
            ],
            options={
                'verbose_name': 'Vendor Mapping',
                'verbose_name_plural': 'Vendor Mappings',
                'db_table': 'vendor_mappings',
                'abstract': False,
                'managed': False,
            },
        ),
        migrations.CreateModel(
            name='UserProfile',
            fields=[
                ('id', models.BigAutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('paperless_token', models.CharField(blank=True, default='', max_length=255)),
                ('paperless_url', models.URLField(blank=True, default='', help_text='Override default Paperless URL')),
                ('paperless_filter_tags', models.CharField(blank=True, default='finance/inbox', help_text="Comma-separated tags to filter documents during extraction (e.g., 'finance/inbox,receipts')", max_length=500)),
                ('firefly_token', models.CharField(blank=True, default='', max_length=255)),
                ('firefly_url', models.URLField(blank=True, default='', help_text='Override default Firefly URL')),
                ('default_source_account', models.CharField(blank=True, default='Checking Account', max_length=255)),
                ('auto_import_threshold', models.FloatField(default=0.85, help_text='Auto-import confidence threshold')),
                ('review_threshold', models.FloatField(default=0.6, help_text='Review confidence threshold')),
                ('syncthing_url', models.URLField(blank=True, default='', help_text='URL to Syncthing web UI for document syncing')),
                ('importer_url', models.URLField(blank=True, default='', help_text='URL to Firefly Importer for bank statement imports')),
                ('llm_enabled', models.BooleanField(default=False, help_text='Enable LLM-assisted categorization')),
                ('ollama_url', models.URLField(blank=True, default='', help_text='Ollama server URL (e.g., http://localhost:11434)')),
                ('ollama_model', models.CharField(blank=True, default='', help_text='Fast model name (e.g., qwen2.5:3b-instruct-q4_K_M)', max_length=100)),
                ('ollama_model_fallback', models.CharField(blank=True, default='', help_text='Fallback model for complex cases (e.g., qwen2.5:7b-instruct-q4_K_M)', max_length=100)),
                ('ollama_timeout', models.IntegerField(default=30, help_text='Request timeout in seconds')),
                ('created_at', models.DateTimeField(auto_now_add=True)),
                ('updated_at', models.DateTimeField(auto_now=True)),
                ('user', models.OneToOneField(on_delete=django.db.models.deletion.CASCADE, related_name='profile', to=settings.AUTH_USER_MODEL)),
            ],
        ),
    ]
