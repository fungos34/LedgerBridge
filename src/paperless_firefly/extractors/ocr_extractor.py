...
DATE_PATTERNS = [
    ...
    # German month names
    (r'\b(\d{1,2})\.\s*(Januar|Februar|März|April|Mai|Juni|Juli|August|September|Oktober|November|Dezember)\s*(\d{4})\b', None, 'german_month'),
]
...
INVOICE_PATTERNS = [
    ...
    (r'\b(?:RE|R|INV|INVOICE|Rechnung|Rechnungsnr\.?|Rechnungsnummer|Beleg-?Nr\.?)[:\s#-]*([A-Z0-9]+-?\d{4,}(?:-\d+)?)\b', 'invoice_number', 0.9),
...
def parse_date_match(match: re.Match, date_format: Optional[str], pattern_type: str) -> Optional[str]:
...
class OCRTextExtractor(BaseExtractor):
        ...
        for pattern, date_format, pattern_type in DATE_PATTERNS:
            for match in re.finditer(pattern, content, re.IGNORECASE):
                ...
                if parsed_date:
                    ...
                    if any(kw in context for kw in ['datum', 'date', 'rechnungsdatum', 'belegdatum']):
    ...
    def _extract_amount(self, content: str, currency: Optional[str] = None) -> Optional[dict[str, Any]]:
        """
