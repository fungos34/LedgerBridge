...
def generate_external_id(
    ...
    if not source_hash or len(source_hash) < 16:
        raise ValueError(f"source_hash must be at least 16 characters, got: {len(source_hash) if source_hash else 0}")
...
def parse_external_id(external_id: str) -> ExternalIdComponents:
    ...
    if not external_id.startswith("paperless:"):
        raise ValueError(f"Invalid external_id prefix, expected 'paperless:', got: {external_id[:20]}")
