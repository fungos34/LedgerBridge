...
class TestUBLVariants:
    ...
    def test_peppol_bis(self, extractor):
        ...
        peppol_xml = SAMPLE_UBL_XML.replace(
            "urn:xoev-de:kosit:standard:xrechnung_2.0", "urn:fdc:peppol.eu:2017:poacc:billing:3.0"
        )
...
class TestExtractorRouterIntegration:
    ...
    def test_router_uses_einvoice_for_xml(self):
        ...
        assert (
            "einvoice" in result.provenance.extraction_strategy
            or "cii" in result.provenance.extraction_strategy
            or "ubl" in result.provenance.extraction_strategy
        )
