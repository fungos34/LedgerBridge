        ...
        if not settings.configured:
            settings.configure(
                ...
                TEMPLATES=[{
                    ...
                    'DIRS': [
                        Path(__file__).parent.parent / 'src' / 'paperless_firefly' / 'review' / 'web' / 'templates'
                    ],
                    'APP_DIRS': True,
                    'OPTIONS': {
                        'context_processors': [
...
class TestImportQueue:
    ...
    def test_import_queue_shows_approved_extractions(self, tmp_path):
        ...
        # Verify data exists
        conn = sqlite3.connect(str(state_db))
        row = conn.execute("SELECT * FROM extractions WHERE review_decision = 'ACCEPTED'").fetchone()
        conn.close()
...
class TestMobileFriendly:
    ...
    def test_viewport_meta_in_templates(self):
        ...
        template_dir = Path(__file__).parent.parent / 'src' / 'paperless_firefly' / 'review' / 'web' / 'templates'
